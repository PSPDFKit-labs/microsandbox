...
Start egress interception with callback hooks. Connects to the sandbox's egress socket and starts processing events in the background. Returns an `EgressInterceptHandle` immediately.
...
|--------|------|-------------|
| done | `Promise<void>` | Resolves when the loop ends naturally (sandbox stopped) |
| stop() | `() => Promise<void>` | Signal the loop to stop and wait for completion |
