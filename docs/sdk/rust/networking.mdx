...
```rust
async fn intercept<F, G>(
    &mut self,
    on_request: F,
    on_response: G,
) -> io::Result<()>
where
    F: FnMut(&mut HttpRequest, &EgressContext) -> Result<Option<RequestAction>, Box<dyn Error>>,
    G: FnMut(&mut HttpResponse, &HttpRequest, &EgressContext) -> Result<Option<HttpResponse>, Box<dyn Error>>,
```

Run the interception event loop with user-provided hooks. Blocks until the sandbox stops or the connection closes.
